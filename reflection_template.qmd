...
# one edit: Use read_csv explicitly so this chunk proves "import" without relying on library(tidyverse) being already attached
...
# Lab 4 · Q7 — tidy to long + relabel/relevel
plot_data <- ca_childcare |>
  select(region, study_year, mc_infant, mc_toddler, mc_preschool) |>
  pivot_longer(
    cols      = c(mc_infant, mc_toddler, mc_preschool),
...
ca_childcare <- counties |>
  ...
  inner_join(                             # EDIT: qualify to emphasize dplyr join
    childcare_costs |>
...
# Lab 4 · Q3
ca_childcare <- ca_childcare |>
  left_join(                              # EDIT: qualify to emphasize dplyr join
    tax_rev |>
      rename(
...
plot_data <- ca_childcare |>
  select(region, study_year, mc_infant, mc_toddler, mc_preschool) |>
  pivot_longer(                             # EDIT: qualify to foreground tidyr pivot
    cols      = c(mc_infant, mc_toddler, mc_preschool),
    names_to  = "age_group",
    values_to = "price_weekly"
  )
...
mhi_by_region <- ca_childcare |>
  ...
  pivot_wider(                              # EDIT: qualify to foreground tidyr pivot
...
ggplot(surveys, aes(x = weight, y = hindfoot_length)) +
  geom_point(alpha = 0.3) +                 # reduces overplotting (clarity)
  facet_wrap(~ species) +                   # many smalls; numeric × numeric across groups
  labs(
...
ggplot(surveys, aes(x = species, y = weight)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.4) +                      # improves visibility of points
  labs(
    x     = "Species",
    y     = "Weight (grams)",
    title = "Rodent Weights by Species"
  ) +
  theme(axis.text.x = element_text(angle = 45))   # improves label readability
```
...
# Lab 3 · Challenge 2
ggplot(teacher_evals_compare, aes(x = sen_level, fill = set_level)) +
  geom_bar(position = "fill") +                     # compositional view
  scale_fill_manual(values = c("excellent" = "#9834eb", "standard" = "#d46c22")) +
...
teacher_evals_clean |>
  ...
  count(academic_degree, name = "n_instructors")
```
